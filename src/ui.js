...
<section class="card"><h3>Outbound</h3><label>Proxy</label><input id="outbound" placeholder="socks5://user:pass@host:1080"><label>Mode</label><select id="mode"><option>adaptive</option><option>proxy-first</option><option>direct-first</option><option>proxy-only</option><option>direct</option></select><label>Dial race</label><input id="dialRace" type="number" min="1" max="4"><div class="hint">Adaptive mode tries normal direct traffic first, hedges stalled Cloudflare-backed web traffic through discovered Cloudflare edge addresses, then uses ProxyIP and your external proxy only as later fallbacks.</div></section>
<section class="card wide"><h3>Adaptive egress</h3><div class="row"><label class="check"><input id="adaptiveEdge" type="checkbox"> Enable Cloudflare edge discovery</label></div><div class="metrics"><div><label>Hedge delay (ms)</label><input id="hedgeDelayMs" type="number" min="100" max="5000"></div><div><label>Direct first-byte timeout (ms)</label><input id="firstByteTimeoutMs" type="number" min="1000" max="15000"></div><div><label>Edge first-byte timeout (ms)</label><input id="edgeFirstByteTimeoutMs" type="number" min="750" max="10000"></div><div><label>Edge race</label><input id="edgeRace" type="number" min="1" max="4"></div></div><div class="hint">The edge path is only considered after a stall and only when DoH shows that the application hostname resolves into Cloudflare's published IP space. Active edge addresses are discovered dynamically instead of copying a fixed ProxyIP list. Failed edge candidates receive an in-memory cooldown.</div></section>
...
const ids=['uuid','path','trojanPassword','subscriptionName','outbound','mode','dialRace','hedgeDelayMs','firstByteTimeoutMs','edgeFirstByteTimeoutMs','edgeRace','adaptiveEdge','proxyIp','preferred','routes','enableWs','enableXhttp','blockPrivate','disableIpv6','allowPathOverride'];
const numericIds=new Set(['dialRace','hedgeDelayMs','firstByteTimeoutMs','edgeFirstByteTimeoutMs','edgeRace']);
