...
function set(id,v){const e=document.getElementById(id);if(e.type==='checkbox')e.checked=!!v;else if(Array.isArray(v))e.value=v.map(x=>typeof x==='string'?x:(x.host+(x.port?':'+x.port:'')+(x.name?'#'+x.name:''))).join(', ');else if(id==='routes'&&Array.isArray(v))e.value=v.map(x=>x.pattern+'='+x.policy).join('; ');else e.value=v??''}
